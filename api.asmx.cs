...
namespace StahrDBAPI {
	...
	/// 
	///		Author:			Mike Stahr
    ///		Group Member: Phuong Ho, Ziyang Song
	///		Created:		25-11-2017
	...
	public class api : System.Web.Services.WebService {
		...
		*/
        ...
        /* Example of a connection string that points to the AP database on the localdb SQL Server
		  <connectionStrings>
			<add name="DefaultConnection" connectionString="Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=CourseInformation;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=True" providerName="System.Data.SqlClient" />
		  </connectionStrings>
		*/
        ...
        #endregion
        [WebMethod]
        public void addStudent(string FirstName,string LastName, int credit, string classYr, bool honor, string Email, double GPA) {
            addParam("@FirstName",FirstName);
            addParam("@LastName",LastName);
            addParam("@credit",credit);
            addParam("@class",classYr);
            addParam("@honor",honor);
            addParam("@Email",Email);
            addParam("@GPA",GPA);
            sqlExec("spAddStudent");
            getStudent();
        }
        [WebMethod]
        public void getStudent()
        {
            send("spGetStudent", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void updateStudent(int studentId, string FirstName, string LastName, int credit, string classYr, bool honor, string Email, double GPA)
        {
        ...
        {
            addParam("@studentId", studentId);
            sqlExec("spDeleteStudent");
            getStudent();
        }

        [WebMethod]
        public void getCountByProfessorCourse(int professorId){
            addParam("@professorId", professorId);
            send("spGetCountByProfessorCourse", serializeStyle.DATA_TABLE);
        }
        [WebMethod]
        public void getCourseByStudentId(int studentId)
        {
            addParam("@studentId", studentId);
            send("spGetCourseByStudentId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getHonorStudent(){
            send("spGetHonorStudent", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getHonorStudentByCourseId(int courseId)
        {
            addParam("@courseId", courseId);
            send("spGetHonorStudentByCourseId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getNumberofStudentsByCourseId(int courseId)
        {
            addParam("@courseId", courseId);
            send("spGetNumberofStudentsByCourseId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getProfessorNoEmail()
        {
            send("spGetProfessorNoEmail", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getProfessorNoPhoneByDepartmentId(int departmentId)
        {
            addParam("@departmentId", departmentId);
            send("spGetProfessorNoPhoneByDepartmentId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getProfessorWithNoCourses()
        {
            send("spGetProfessorWithNoCourses", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentByCourseId(int courseId)
        {
            addParam("@courseId", courseId);
            send("spGetStudentByCourseId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentByProfessorId(int professorId)
        {
            addParam("@professorId", professorId);
            send("spGetStudentByProfessorId", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentInNoCourse()
        {
            send("spGetStudentInNoCourse", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentUnderCertainGPA(float gpa)
        {
            addParam("@setGPA", gpa);
            send("spGetStudentUnderCertainGPA", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getCourseNotFullCpacity()
        {
            send("spCourseNotFullCpacity", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getProfessorHavingDifferentLocation()
        {
            send("spGetProfessorHavingDifferentLocation", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentsTakingMultiCourseSameProfessor(int studentId)
        {
            addParam("@studentId", studentId);
            send("spGetStudentsTakingMultiCourseSameProfessor", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentsGpaGreaterSameDepartment(int studentId)
        {
            addParam("@studentId", studentId);
            send("spGetStudentsGpaGreaterSameDepartment", serializeStyle.DATA_TABLE);
        }

        [WebMethod]
        public void getStudentScripTakingMultipleCourse()
        {
            send("spGetStudentScriptTakingMultipleCourses", serializeStyle.DATA_TABLE);
        }
        [WebMethod]
        public void getHighestHonoredStudentsDepartment()
        {
            send("spGetHighestHonoredStudentsDepartment", serializeStyle.DATA_TABLE);
        }
        [WebMethod]
        public void getprofessorCreditHours()
        {
            send("spGetProfessorCreditHours", serializeStyle.DATA_TABLE);
        }
    }
}
